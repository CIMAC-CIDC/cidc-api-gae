...
def test_grant_download_access(monkeypatch):
    ...
    def set_iam_policy(policy):
        bindings = policy.bindings
        assert len(bindings) == 2
        expression = bindings[0]["condition"]["expression"]
        print(expression)
...
def test_revoke_download_access(monkeypatch):
    binding = {
        "condition": {
            "title": "Conditional download access for test@email.com",
            "description": "Auto-generated by the CIDC API on 2020-08-27 22:39:46.946004",
            "expression": '(resource.name.startsWith("projects/_/buckets/cidc-data-staging/objects/10021/wes") && request.time < timestamp("2020-10-26T00:00:00Z")) && (resource.name.startsWith("projects/_/buckets/cidc-data-staging/objects/10021/cytof") && request.time < timestamp("2020-10-26T00:00:00Z"))',
        },
        "members": {"user:test@email.com"},
        "role": GOOGLE_DOWNLOAD_ROLE,
    }
    ...
    def set_iam_policy(policy):
        ...
        assert "10021/wes" not in policy.bindings[0]["condition"]["expression"]
    ...
    binding["condition"][
        "expression"
    ] = '(resource.name.startsWith("projects/_/buckets/cidc-data-staging/objects/10021/cytof") && request.time < timestamp("2020-10-26T00:00:00Z"))'
    _mock_gcloud_storage([binding, other_binding], set_iam_policy, monkeypatch)
    revoke_download_access(EMAIL, "10021", "wes")
    ...
    binding["condition"]["expression"] = (
        '(resource.name.startsWith("projects/_/buckets/cidc-data-staging/objects/10021/cytof") && request.time < timestamp("2020-10-26T00:00:00Z"))'
        ' || (resource.name.startsWith("projects/_/buckets/cidc-data-staging/objects/10021/wes") && request.time < timestamp("2020-10-26T00:00:00Z"))'
        ' || (resource.name.startsWith("projects/_/buckets/cidc-data-staging/objects/10021/wes") && request.time < timestamp("2020-10-26T00:00:00Z"))'
    )
    _mock_gcloud_storage([binding, other_binding], set_iam_policy, monkeypatch)
    revoke_download_access(EMAIL, "10021", "wes")
